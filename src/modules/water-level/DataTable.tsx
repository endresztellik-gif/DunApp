...
const getTrendIcon = (current: number, previous: number | null) => {
  ...
  if (diff > 10)
    return <TrendingUp className="h-4 w-4" style={{ color: 'var(--status-alert-text)' }} />;
  if (diff < -10)
    return <TrendingDown className="h-4 w-4" style={{ color: 'var(--color-dun-ok-500)' }} />;
  return <Minus className="h-4 w-4" style={{ color: 'var(--text-tertiary)' }} />;
};
...
const getLevelColor = (level: number, station: WaterLevelStation | null): React.CSSProperties => {
  ...
  if (station.dangerLevelCm && level >= station.dangerLevelCm)
    return { color: 'var(--status-alert-text)', fontFamily: 'var(--font-data)' };
  if (station.alertLevelCm && level >= station.alertLevelCm)
    return { color: 'var(--color-dun-amber-400)', fontFamily: 'var(--font-data)' };
  if (station.highWaterLevelCm && level >= station.highWaterLevelCm)
    return { color: 'var(--color-dun-amber-200)', fontFamily: 'var(--font-data)' };
        ...
        <table className="min-w-full">
          ...
          >
            <tr>
              ...
              <th
                scope="col"
                className="px-6 py-3 text-center text-xs font-medium tracking-wider uppercase"
                style={{ color: 'var(--text-secondary)' }}
              >
