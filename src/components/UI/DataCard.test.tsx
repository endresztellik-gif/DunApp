...
describe('DataCard - Rendering', () => {
  it('renders icon, label, value, and unit', () => {
    render(<DataCard icon={Thermometer} label="Hőmérséklet" value="25.5" unit="°C" />);
  ...
  it('displays – when value is null', () => {
    render(<DataCard icon={Thermometer} label="Hőmérséklet" value={null} unit="°C" />);

    expect(screen.getByText('–')).toBeInTheDocument();
  });

  it('displays – when value is undefined', () => {
    render(<DataCard icon={Thermometer} label="Hőmérséklet" value={undefined as any} unit="°C" />);
  ...
  it('handles zero value correctly', () => {
    render(<DataCard icon={Thermometer} label="Hőmérséklet" value={0} unit="°C" />);
...
describe('DataCard - Module Colors', () => {
  it('applies meteorology accent color to icon by default', () => {
    const { container } = render(<DataCard icon={Thermometer} label="Test" value="10" unit="°C" />);
  ...
  it('applies meteorology color when specified', () => {
    const { container } = render(
      <DataCard icon={Thermometer} label="Test" value="10" unit="°C" moduleColor="meteorology" />
    );
  ...
  it('applies water level color when specified', () => {
    const { container } = render(
      <DataCard icon={Droplets} label="Test" value="10" unit="cm" moduleColor="water" />
    );
  ...
  it('applies drought color when specified', () => {
    const { container } = render(
      <DataCard icon={Wind} label="Test" value="10" unit="%" moduleColor="drought" />
    );
...
describe('DataCard - Accessibility', () => {
  it('has region role', () => {
    render(<DataCard icon={Thermometer} label="Hőmérséklet" value="25" unit="°C" />);
...
describe('DataCard - Styling', () => {
  ...
  it('accepts custom className', () => {
    const { container } = render(
      <DataCard icon={Thermometer} label="Test" value="10" unit="°C" className="custom-class" />
    );
...
describe('DataCard - Different Icon Types', () => {
  it('renders Thermometer icon', () => {
    render(<DataCard icon={Thermometer} label="Test" value="10" unit="°C" />);
    expect(screen.getByText('Test')).toBeInTheDocument();
  });

  it('renders Droplets icon', () => {
    render(<DataCard icon={Droplets} label="Test" value="10" unit="%" />);
    expect(screen.getByText('Test')).toBeInTheDocument();
  });

  it('renders Wind icon', () => {
    render(<DataCard icon={Wind} label="Test" value="10" unit="km/h" />);
    expect(screen.getByText('Test')).toBeInTheDocument();
  });
});
